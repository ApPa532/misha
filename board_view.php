...
<?php
	...
	//글 목록 페이지로 부터 레코드 일련번호와 페이지 번호를 각각 전달받아 변수에 저장
	$num  = $_GET["num"];
	$page  = $_GET["page"];
	...
	//str_replace() 함수는 HTML의 &nbsp;로 변경해주는 함수
	...
	//조회수 값 증가와 DB 업데이트
	$new_hit = $hit + 1;
	$sql = "update board set hit=$new_hit where num=$num";  //필드를 업데이트 해주기 위한 변수 
